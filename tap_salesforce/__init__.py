...
def is_property_selected(  # noqa: C901  # ignore 'too complex'
    stream_name, metadata_map, breadcrumb
) -> bool:
    """
...
def pop_deselected_schema(schema, stream_name, breadcrumb, metadata_map):
    ...
    for property_name, val in list(schema.get("properties", {}).items()):
        property_breadcrumb = tuple(list(breadcrumb) + ["properties", property_name])
...
async def sync_catalog_entry(sf, catalog_entry, state):
    ...
    if job_id:
        with metrics.record_counter(stream) as counter:
            ...
            state = singer.write_bookmark(
                state, catalog_entry["tap_stream_id"], replication_key, bookmark
            )
            singer.write_state(state)
    else:
